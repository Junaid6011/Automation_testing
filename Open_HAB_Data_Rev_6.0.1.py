...
APARTMENTS = {
    ...
    "1_bedroom": {
        ...
        "rooms": ["powder_room", "master_bedroom","master_bathroom", "living_room", "dressing_room","Utility","kitchen"],
    }
}
...
for apt, cfg in APARTMENTS.items():
    ...
    # compute initial people count (total_in - total_out)
    latest_data[apt]["peoplecounter"]["count"] = latest_data[apt]["peoplecounter"]["total_in"] - latest_data[apt]["peoplecounter"]["total_out"]
...
def publish_peoplecounter(apt_name):
    ...
    payload = {
        ...
        "people_count": pc.get("total_in", 0) - pc.get("total_out", 0),
    ...
    while True:
        for apt in APARTMENTS:
            with latest_data_lock:
                ...
                # maintain computed count field (in - out)
                latest_data[apt]["peoplecounter"]["count"] = latest_data[apt]["peoplecounter"]["total_in"] - latest_data[apt]["peoplecounter"]["total_out"]
